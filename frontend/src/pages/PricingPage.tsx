...
import { getPlans, verifyPayment } from '../services/billingService';
  ...
  const handleUpgrade = async (tier: PlanTier) => {
    ...
    try {
      ...
      const result = await verifyPayment({
        ...
        razorpay_signature: 'placeholder_sig'
      });
        ...
        >
          <div className="bg-gradient-to-r from-red-50 to-rose-50 rounded-2xl border border-red-100 p-8 text-center">
            ...
            <p className="text-text-secondary text-sm">
              Payments in ₹ INR via Razorpay. UPI, cards, netbanking & wallets accepted. GST invoice available on request.
            </p>
          </div>
        </motion.div>
      </div>
