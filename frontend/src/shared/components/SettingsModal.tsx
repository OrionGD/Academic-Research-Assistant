  ...
  const Toggle = ({ on, onToggle, disabled }: { on: boolean; onToggle: () => void; disabled?: boolean }) => (
    <button 
      ...
      className={cn(
        ...
        on ? "bg-accent shadow-[0_0_10px_rgba(129,140,248,0.5)]" : "bg-white/10",
        disabled && "opacity-30 cursor-not-allowed"
      )}
    >
      <div className={cn(
        "absolute top-0.5 w-4 h-4 bg-white rounded-full shadow-lg transition-all duration-300 ease-spring", 
    ...
    <AnimatePresence>
      {settingsOpen && (
        <>
          ...
          >
            <div className="bg-[#050508] border border-white/10 rounded-[32px] shadow-2xl w-full max-w-lg pointer-events-auto overflow-hidden flex flex-col max-h-[90vh]">
              {/* Header */}
              <div className="flex items-center justify-between px-8 py-6 border-b border-white/5 bg-white/[0.02]">
                ...
                <button 
                  onClick={() => setSettingsOpen(false)} 
                  className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-text-dim hover:text-text-primary hover:bg-white/10 transition-all"
                >
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between p-4 bg-white/[0.02] border border-white/5 rounded-2xl hover:bg-white/[0.04] transition-all opacity-50 cursor-not-allowed">
                      <div className="flex items-center gap-4">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between p-4 bg-white/[0.02] border border-white/5 rounded-2xl">
                    ...
                    <select 
                      ...
                      className="bg-white/5 border border-white/10 text-text-primary rounded-xl px-4 py-2 text-[11px] font-bold outline-none focus:border-accent/40 transition-all uppercase tracking-widest"
                    >
