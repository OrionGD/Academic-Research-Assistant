...
export default function DocumentViewerModal({ isOpen, onClose, documentId }: DocumentViewerModalProps) {
  ...
  // Construct absolute URL for the iframe.
  // Since we use session cookies, the browser will automatically include them
  // in the iframe request if it's on the same domain. However, to be safe
  // and handle potential cross-site cookie issues, we fetch the content via blob
  ...
  useEffect(() => {
    ...
    const fetchBlob = async () => {
      try {
        setLoading(true);
        // apiClient implicitly handles session cookies (withCredentials: true)
