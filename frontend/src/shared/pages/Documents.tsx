...
export default function DocumentsPage() {
  ...
  // Auto-open viewer if highlight is present
  useEffect(() => {
    if (highlightId) {
      setViewerDocumentId(highlightId);
    }
  }, [highlightId]);
  ...
  // Handle scrolling to highlighted document
  useEffect(() => {
    ...
    <div className="h-full bg-background overflow-y-auto">
      <div className="max-w-[1400px] mx-auto px-8 py-10 space-y-10">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-text-primary tracking-tight">Research Library & Sources</h1>
            <p className="text-text-dim mt-1">Ingest, manage, and explore your research knowledge base.</p>
          </div>
          <button 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {filteredDocs.map((doc: any) => (
              <motion.div 
                ...
                className={cn(
                  "bb-premium-card group hover:border-accent/30 p-6 space-y-4 cursor-pointer transition-all duration-500",
                  doc.id === highlightId ? "border-accent shadow-[0_0_20px_var(--color-accent-glow)] ring-2 ring-accent/50 scale-[1.02]" : "border-white/10"
                )}
              ...
              >
                <div className="flex items-start justify-between">
                  <div className={cn(
                    ...
                    doc.status === 'completed' ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.1)]" : 
                    doc.status === 'processing' ? "bg-accent/10 border-accent/20 text-accent animate-accent-pulse shadow-[0_0_20px_var(--color-accent-glow)]" : 
                  ...
                  <div className="flex items-center gap-1">
                    <button 
                      ...
                      className="p-2 text-text-dim hover:text-accent hover:bg-accent/10 rounded-lg transition-all"
                      title="View Document"
                    >
                      <Eye size={16} />
                    </button>
