...
export default function SearchPage() {
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    setIsSearching(true);
    try {
      const data = await searchService.search(query, undefined, 1, 10);
      setResults(data);
      if (data.length === 0) {
            ...
            {isSearching ? (
              ...
              results.map((result, i) => (
                <motion.div 
                  ...
                  className="bb-premium-card p-6 lg:p-8 space-y-4 lg:space-y-6 hover:border-accent/40 cursor-pointer group bg-accent/[0.02] border-accent/10 rounded-[1.5rem] lg:rounded-[2rem]"
                >
                  <div className="flex items-center justify-between">
                     ...
                     <div className="w-8 h-8 lg:w-10 lg:h-10 rounded-lg lg:rounded-xl bg-accent/5 flex items-center justify-center text-text-muted group-hover:text-accent transition-all">
