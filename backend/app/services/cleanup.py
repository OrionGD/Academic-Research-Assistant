...
async def cleanup_stale_documents(max_age_hours: int = 2):
    ...
    for doc in docs:
        ...
        # 3. Delete Chat History for this document
        try:
            await db.chat_history.delete_many({"document_id": doc_id})
            await db.chats.delete_many({"document_id": doc_id})
        except Exception:
            pass
            
    # 4. Delete from DB (Documents)
    await db.documents.delete_many(query)
    
    # 5. Catch any chats directly by sessionId for the stale documents
    for doc in docs:
        sid = doc.get("sessionId")
        if sid and sid != "public":
            await db.chat_history.delete_many({"sessionId": sid})
            await db.chats.delete_many({"sessionId": sid})

    logger.info(f"Successfully cleaned up {len(docs)} stale documents and associated history.")
