...
def extract_text_from_pdf(file_content: bytes) -> tuple[str, int]:
    ...
    if full_text.strip():
        ...
        return full_text, page_count
    logger.warning("[Extraction] OCR returned empty text. Document may be unreadable.")
...
async def process_document_pipeline(
    ...
    if not full_text.strip():
        raise ValueError(
            ...
            "and OCR fallback also failed. Please upload a text-based PDF, "
            "or ensure Tesseract OCR + Poppler are installed on this server."
        )
    ...
    # Step 2: Section-aware chunking with page numbers
    chunks_data = chunk_text_with_sections(full_text, page_count)
    if not chunks_data:
        raise ValueError("No chunks generated from document.")

    logger.info(f"[Process] Generated {len(chunks_data)} chunks")

    # Step 3: Batch embed with Gemini API (best-effort)
    embeddings = []
    try:
        ...
        if embeddings and len(embeddings) > 0:
            ...
            if len(valid_pairs) != len(chunks_data):
                missing = len(chunks_data) - len(valid_pairs)
                logger.warning(f"[Process] {missing} embeddings returned None. Filtering invalid entries.")
                chunks_data = [p[0] for p in valid_pairs]
                embeddings = [p[1] for p in valid_pairs]

            if not embeddings:
                logger.warning("[Process] No valid embeddings generated; skipping vector storage.")
            else:
                ...
                for i, emb in enumerate(embeddings):
                    if len(emb) != EXPECTED_EMBEDDING_DIM:
                        logger.error(
                            f"[Process] Embedding dimension mismatch at index {i}: "
                            f"expected {EXPECTED_EMBEDDING_DIM}, got {len(emb)}. "
                            f"Skipping vector storage."
                        )
                        dim_mismatch = True
                        break

                if not dim_mismatch:
                    logger.info(f"[Process] Embedded {len(embeddings)} chunks ({len(embeddings[0])}d)")
            ...
            logger.warning("[Process] Embedding generation returned no results; skipping vector storage.")
    except Exception as embed_err:
        logger.warning(f"[Process] Embedding/ChromaDB failed (continuing without vector storage): {embed_err}")
    ...
    return {
        ...
        "embeddingDimensions": len(embeddings[0]) if embeddings and isinstance(embeddings[0], list) else 0,
        "status": "completed",
