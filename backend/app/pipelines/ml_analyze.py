...
def _parse_gemini_response(response: str) -> dict:
    """Extract and parse JSON from Gemini response string."""
    try:
...
def _format_analysis_result(parsed: dict, document_id: str, depth: str, full_text: str, reading_time: int, complexity: str, word_count: int, start_time: float, model_version: str) -> dict:
    """Format the parsed analysis result into the final schema."""
    result = {
    ...
    logger.info(f"[AnalyzePipeline] {model_version} analysis complete doc={document_id} time={processing_time}ms")
    return result
...
async def analyze_document_pipeline(document_id: str, full_text: str, depth: str = "full", title: str = "Research Paper") -> dict:
    ...
    # Step 1: AI-powered analysis (Gemini or Groq based on settings)
    provider = settings.LLM_PROVIDER.lower()

    if depth != "limited":
        # Order of execution based on provider preference
        if provider == "groq":
            # Groq first
            try:
                parsed = await _analyze_with_groq(full_text, title)
                if parsed:
                    return _format_analysis_result(parsed, document_id, depth, full_text, reading_time, complexity, word_count, start, f"groq-{settings.GROQ_CHAT_MODEL}")
            except Exception as e:
                logger.warning(f"Groq analysis failed for {document_id}, trying Gemini fallback: {e}")
            
            # Gemini fallback
            if gemini_client.analysis_client:
                try:
                    ...
                    if parsed:
                        return _format_analysis_result(parsed, document_id, depth, full_text, reading_time, complexity, word_count, start, settings.gemini_analysis_model)
                except Exception as e:
                    logger.warning(f"Gemini fallback failed for {document_id}: {e}")
        else:
            ...
            try:
                ...
                logger.warning(f"Groq fallback failed for {document_id}: {e}")
