...
[![Version](https://img.shields.io/badge/version-2.1.3-blue.svg)](https://github.com/OrionGD/ScholarAI)
...
| **Context Control (Collections)** | Logical grouping of documents into thematic collections to scope AI search and RAG operations to specific project contexts. | Zustand Store, MongoDB filtering |
...
### Session-Isolated Platform Notice

ScholarAI version 2.1.3 operates as a session-isolated open-access platform. The following privacy and security characteristics govern the current release:

- **Ephemeral Sessions:** No registration is required. Every user is assigned a unique, ephemeral session ID upon access.
- **Data Isolation:** Uploaded documents, chat histories, and AI analyses are isolated to your specific session and are not visible to other users.
- **Session Purge:** Users can explicitly clear their entire research context via the "Logout Mission" feature, which purges all session-specific data from the backend and vector stores.
- **Automatic Cleanup:** Periodic system tasks identify and purge stale session data to ensure continuous privacy and optimal system performance.
...
A: No. The current release is open-access and uses ephemeral session IDs to identify you. You can start using all features immediately without registration.

**Q: Are uploaded documents private?**
A: Yes, within the scope of your session. Documents and chat history are isolated to your session ID. Once you explicitly "Logout Mission" or the session expires, all associated data is purged from the system.
...
| | 2026 Q3 | Session-isolated data environments | Completed | Privacy without authentication friction |
| | 2026 Q3 | Context Control (Collections) scoping | Completed | User-defined RAG search context |
| | 2026 Q3 | Automatic session data cleanup | Completed | GDPR-aligned ephemeral data handling |
| | 2026 Q3 | Full User Authentication (OAuth/JWT) | Planned | Persistent accounts and team features |
